...
import numpy as np
try:
    # for reason this doesn't work on dolphin
...
def load(data_dir, subset='train'):
    path = os.path.join(data_dir,'imgnet_32x32.npz')
    print('looking for', path)
    if not os.path.exists(path):
        print(path, 'does not exist')
        if not os.path.exists(data_dir):
            print('creating folder', os.path.abspath(data_dir))
            os.makedirs(data_dir)
        if not os.path.exists(os.path.join(data_dir,'imgnet_32x32.npz')):
            maybe_download_and_extract(data_dir)
            maybe_preprocess(data_dir)
    imagenet_data = np.load(path)
...
class DataLoader(object):
    ...
    def __next__(self, n=None):
        ...
        """
        inds = np.random.randint(self.data.shape[0], size=(n,))
        x = self.data[inds]
